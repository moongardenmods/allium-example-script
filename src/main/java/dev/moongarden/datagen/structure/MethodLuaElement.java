...
public class MethodLuaElement extends AbstractExecutableLuaElement {
    ...
    public MethodLuaElement(EMethod method, List<EClass<?>> visible) {
        super(method, visible);
        if (!method.rawReturnType().raw().isPrimitive() && !method.rawReturnType().raw().isArray() && !visible.contains(method.rawReturnType())) visible.add(method.rawReturnType());
